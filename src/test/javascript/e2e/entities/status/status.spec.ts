...
describe('Status e2e test', () => {
    ...
    it('should load Statuses', async () => {
        ...
        expect(await statusComponentsPage.getTitle()).to.eq('releaseVitalsApp.status.home.title');
    });
    ...
    it('should load create Status page', async () => {
        ...
        expect(await statusUpdatePage.getPageTitle()).to.eq('releaseVitalsApp.status.home.createOrEditLabel');
    ...
    it('should create and save Statuses', async () => {
        ...
        await statusComponentsPage.clickOnCreateButton();
        await promise.all([
            statusUpdatePage.setFromStatusInput('fromStatus'),
            statusUpdatePage.setToStatusInput('toStatus'),
            statusUpdatePage.setTimeChagedInput('01/01/2001' + protractor.Key.TAB + '02:30AM'),
            statusUpdatePage.issueSelectLastOption()
        ]);
        expect(await statusUpdatePage.getFromStatusInput()).to.eq('fromStatus');
        expect(await statusUpdatePage.getToStatusInput()).to.eq('toStatus');
        expect(await statusUpdatePage.getTimeChagedInput()).to.contain('2001-01-01T02:30');
    ...
    it('should delete last Status', async () => {
        ...
        expect(await statusDeleteDialog.getDialogTitle()).to.eq('releaseVitalsApp.status.delete.question');
